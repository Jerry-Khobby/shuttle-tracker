...
const LoginScreen = () => {
  ...
  return (
    <SafeAreaView className={`flex-1 ${isDarkMode ? 'bg-black' : 'bg-white'} mt-14`}>
      <View className="items-center justify-center space-y-8 px-6">
        {/* Heading */}
        <Text className={`text-3xl font-bold ${headingColor}`}>Bus Tracker</Text>
        ...
        {/* Signin Text */}
        <Text className={`text-2xl font-semibold font-serif ${subheadingColor}`}>
          Sign in to continue
        </Text>
        ...
        <View className="w-full space-y-4">
          ...
          <TouchableOpacity className={`bg-slate-200 py-3 rounded-md h-11 ${isDarkMode ? 'bg-gray-700' : 'bg-slate-200'}`}>
            <Text className={`${isDarkMode ? 'text-white' : 'text-black'} font-bold text-center`}>
              Login as a Driver
            </Text>
          </TouchableOpacity>
        </View>
